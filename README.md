...
- ClassRoom은 약 105 BPM 기준으로 다시 분석한 박자 위상에 맞춰 배치했고, 마지막 3초 이상은 노트가 나오지 않도록 피날레를 정리했습니다.
- ClassRoom은 처음 4초 동안 노트가 없고, `준비 도입 -> 교실 그루브 -> 빌드업 -> 하이라이트 -> 회복 -> 피날레` 구조로 밀도가 오르내립니다.
- Recruitment는 약 92 BPM 기준으로 다시 분석한 박자 위상에 맞춰 배치했고, ClassRoom과 비슷하거나 아주 약간 더 긴장감 있게 설계했습니다.
