...
2. 시작 화면에서 `Easy` 또는 `Normal`을 고릅니다.
3. 화면 중앙의 `Start`를 누른 뒤 `D / F / J / K`로 플레이합니다.
4. 곡이 끝나면 결과창에서 점수, 최대 콤보, 정확도, 판정 수, 랭크를 확인하고 `Replay`로 다시 시작할 수 있습니다.
...
- `index.html`: standalone 실행 파일이며 Easy/Normal 차트가 내장되어 있습니다.
- `main.wav`: 플레이 음악입니다.
...
- 결과창: Final Score, Max Combo, Accuracy, 판정 수, Rank, Replay
- 키 입력 tick, 성공 hit, 실패 miss 사운드
